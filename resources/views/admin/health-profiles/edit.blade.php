...
<div class="filament-page">
    <div class="container-fluid">
      ...
      action="{{ isset($healthProfile)
        ? route('admin.health-profiles.update',$healthProfile)
        : route('admin.health-profiles.store') }}">
            @csrf
            @isset($healthProfile) @method('PUT') @endisset
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label class="filament-form-label required-field">Nama Profil Kesehatan</label>
                       
                        <input type="text" class="filament-input @error('title') is-invalid @enderror" name="name" value="{{ old('name', $healthProfile->name ?? '') }}" required>
                        @error('title')
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        <label class="filament-form-label">Upload File</label>
                         @if ($healthProfile->file_path)
                        <div class="mb-2 text-sm">
                            <strong>File saat ini:</strong><br>
                            <a href="{{ Storage::url($healthProfile->file_path) }}" 
                            target="_blank"
                            class="text-blue-600 underline flex items-center gap-1">
                                📄 {{ basename($healthProfile->file_path) }}
                            </a>
                        </div>
                        @endif
...
fileInput.addEventListener('change', function () {
    ...
    if (!file) {
        fileName.textContent = 'Tidak ada file baru';
        return;
    }
